...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        time = await self.create_message(message=text_data_json['message'],
...
class ProfileListConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        await self.accept()
        await self.update_user_profiles(status='online')
        await self.channel_layer.group_send(self.group_name, {'type': 'users_list'})

    async def disconnect(self, code):
        await self.channel_layer.group_discard(self.group_name, self.channel_name)
        await self.update_user_profiles(status=datetime.now().strftime("%H:%M, %d-%m-%Y"))
        await self.channel_layer.group_send(self.group_name, {'type': 'users_list'})
    ...
    async def update_user_profiles(self, status):
        await self.update_user_database(status)

    @database_sync_to_async
    def update_user_database(self, status):
